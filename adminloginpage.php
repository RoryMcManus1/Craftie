            ...
            <div class="row">
                <div class="col-2">
                    <div class="form-container" style="top:75px">
                        <form action="includes\adminlogin.php" id="RegForm" method="post">
                            <input type="text" name="adminusername1" placeholder="Username...">
                            <input type="password" name="adminpassword1" placeholder="Password...">
                            <button type="submit" name="adminsubmit1" class="btn"> Confirm </button>
                            <?php
                            if (isset($_GET["error"])) {
                                if ($_GET["error"] == "emptyinputadmin") {
                                    echo "<p>All Fields required!</p>";
                                } else if ($_GET["error"] == "invalidusername1") {
                                    echo "<p>Invalid Username entry, Please try again!</p>";
                                } else if ($_GET["error"] == "incorrectAdminpass") {
                                    echo "<p>Password Incorrect, Please try again!</p>";
                                } else if ($_GET["error"] == "nonadmin") {
                                    echo "<p>Account must be an admin to continue!</p>";
                                }
                            }
                            ?>
                        </form>
