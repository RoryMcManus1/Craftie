...
$sqlread = "SELECT `wishlist`.*, `allbeers`.*, `beerstyles`.*, `ratings`.*
FROM `wishlist` 
	LEFT JOIN `allbeers` ON `wishlist`.`item_id` = `allbeers`.`id` 
	LEFT JOIN `beerstyles` ON `allbeers`.`beerstyle` = `beerstyles`.`style_id` 
	LEFT JOIN `ratings` ON `allbeers`.`overall` = `ratings`.`rating_id`;";
...
<body>
    ...
    <div class="small_container">
        ...
        <div class="row">
            ...
            $results_per_page = 16;
            $sql = 'SELECT `wishlist`.*, `allbeers`.*, `beerstyles`.*, `ratings`.*
            FROM `wishlist` 
                LEFT JOIN `allbeers` ON `wishlist`.`item_id` = `allbeers`.`id` 
                LEFT JOIN `beerstyles` ON `allbeers`.`beerstyle` = `beerstyles`.`style_id` 
                LEFT JOIN `ratings` ON `allbeers`.`overall` = `ratings`.`rating_id`;';
            ...
            while ($row = mysqli_fetch_array($result)) {
                ...
                echo "
                   ...
                   <input type='hidden' name='itemid' value='$iddata'>
                   <input type='hidden' name='wishlistid' value='$wishlistid'>

                   
                   <button type='submit' style='margin-left:32px;'name='WLremove' class='btn'>Remove</button>
                   </form>
                     </div> 
                    </a>";
            }
        ...
        <div class="row">
            <span class="page-btn">
                <a href="wishlist.php?page=<?= $Previous; ?>" aria-label="Previous">
                    <span aria-hidden="true">Previous &laquo; </span>
                </a>
                <?php
                for ($i = 1; $i <= $page; $i++) : ?>
                    <a class="page-btn" href="wishlist.php?page=<?= $i; ?>"><?= $i; ?></a>
                <?php endfor; ?>

                <a class="page-btn" href="wishlist.php?page=<?= $Next; ?>" aria-label="Next">
